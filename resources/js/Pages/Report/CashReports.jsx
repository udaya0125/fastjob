...
// import { Calendar, X, DollarSign, TrendingUp } from "lucide-react";
...
//     // Fetch only cash visitors
//     useEffect(() => {
//         const fetchCashVisitors = async () => {
//             try {
//                 setLoading(true);
//                 const response = await axios.get(route("ourvisitors.cash"));
//                 setCashVisitors(response.data);
...
//                 setTotalCash(total);
//             } catch (error) {
//                 console.error("Error fetching cash visitors", error);
//             } finally {
//                 setLoading(false);
//             }
//         };
...
//             setFilteredVisitors(cashVisitors);
//             setIsFiltered(false);
//             return;
//         }

//         const filtered = cashVisitors.filter((visitor) => {
...
//     // Calculate total for filtered visitors
...
//                                         className="w-full sm:w-40 pl-9 pr-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-1 focus:ring-green-500 focus:border-green-500"
...
//                 {!loading && !isFiltered && cashVisitors.length === 0 && (
//                     <div className="text-center py-16 bg-white rounded-xl border border-gray-200">
//                         <DollarSign
//                             size={48}
...
const CashReports = () => {
    ...
    const [yearlyCash, setYearlyCash] = useState(0);
    ...
    useEffect(() => {
        const fetchCashVisitors = async () => {
            try {
                ...
                // Calculate cash statistics (only from income field, not salary) - SAME LOGIC
                const total = response.data.reduce((sum, visitor) => {
                    return sum + (parseFloat(visitor.income) || 0);
                }, 0);
                setTotalCash(total);
                ...
                // Calculate monthly cash (current month) - only from income field - SAME LOGIC
                ...
                setMonthlyCash(monthlyTotal);

                // Calculate yearly cash (current year) - only from income field - SAME LOGIC
    ...
    // Calculate filtered total cash - SAME LOGIC
    const calculateFilteredTotal = useMemo(() => {
        return filteredVisitors.reduce((sum, visitor) => {
            return sum + (parseFloat(visitor.income) || 0);
        }, 0);
    ...
    // Calculate filtered monthly cash based on date filter - SAME LOGIC
    const calculateFilteredMonthlyCash = useMemo(() => {
    ...
    // Calculate filtered yearly cash based on date filter - SAME LOGIC
    const calculateFilteredYearlyCash = useMemo(() => {
        if (!isFiltered || !startDate || !endDate) return 0;
